...
const LCTTranslationCacheBg = (() => {
  ...
  async function get(videoId, langCode) {
    const CacheKeys = getCacheKeys();
    const key = CacheKeys.translation(videoId, langCode);
    const result = await chrome.storage.local.get(key);
    return result[key] || null;
  }

  async function set(videoId, cues, fingerprint, langCode) {
    ...
    const key = CacheKeys.translation(videoId, langCode);
    const entry = { cues, fingerprint, langCode: langCode || CacheKeys.DEFAULT_LANG, timestamp: Date.now() };
    try {
