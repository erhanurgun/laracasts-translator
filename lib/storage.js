...
const Storage = {
  defaults: {
    ...
    bgOpacity: 0.75,
    blurOriginal: false,
    // v0.5.0: kullanıcı tercihleri (uiLanguage varsayılan EN, v0.5.5)
    uiLanguage: 'en',
    targetLanguage: 'tr',
  ...
  _cacheKey(videoId, langCode) {
    return self.LCTCacheKeys
      ? self.LCTCacheKeys.translation(videoId, langCode)
      : `translation_${videoId}_${langCode || 'tr'}`;
  },

  async getCachedTranslation(videoId, langCode) {
    const key = this._cacheKey(videoId, langCode);
    const result = await chrome.storage.local.get(key);
    return result[key] || null;
  },

  async setCachedTranslation(videoId, cues, fingerprint, langCode) {
    const key = this._cacheKey(videoId, langCode);
    const entry = { cues, fingerprint, langCode: langCode || 'tr', timestamp: Date.now() };
    try {
